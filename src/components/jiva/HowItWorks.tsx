...
const HowItWorks = () => {
  ...
  return (
    <section
      id="how-it-works"
      ref={ref}
      className="fade-in-section bg-earth py-20 sm:py-28 px-4 sm:px-6"
    >
            ...
            {steps.map((step, i) => (
              <div key={step.title} className="flex gap-6 relative">
                ...
                <div className="relative z-10 flex-shrink-0 w-10 h-10 rounded-full bg-soil border-2 border-copper flex items-center justify-center">
                  <span className="font-mono-dm text-copper text-xs font-bold">
                    {i + 1}
                  </span>
                </div>

                <div className="pt-1">
                  <h3 className="font-playfair text-cream text-lg font-semibold mb-1">
                    {step.title}
                  </h3>
                  <p className="font-source-serif text-parchment text-sm leading-relaxed">
                    {step.desc}
                  </p>
                </div>
              </div>
